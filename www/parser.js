    ...
    sgvizler.parser = {
        ...
        countRowsSparqlJSON: function (stable) {
            var count = null;
            if (stable.results.bindings !== undefined) {
                count = stable.results.bindings.length;
            }
            return count;
        },
        ...
        SparqlJSON2GoogleJSON: function (stable) {
            ...
            for (c = 0; c < scols.length; c += 1) {
                ...
                while (srows[r][scols[c]] === undefined && r + 1 < srows.length) { r += 1; }
                if (srows[r][scols[c]] !== undefined) {
            ...
            for (r = 0; r < srows.length; r += 1) {
                ...
                for (c = 0; c < scols.length; c += 1) {
                    gvalue = null;
                    if (srow[scols[c]] !== undefined &&
                            srow[scols[c]].value !== undefined) {
        ...
        getGoogleJsonDatatype: function (stype, sdatatype) {
            ...
            if (stype !== undefined && (stype === 'typed-literal' || stype === 'literal')) {
